\newcommand{\drawBlockedwarp}[5]{
  %%
  %% Draw a warp coverage with blocked layout
  %%
  %% Warp TL: pre defined top-left coordinate of the warp
  ...
  \pgfmathsetmacro{\warpSizeM}{\sizePerThreadM*\threadsPerWarpM}
  \pgfmathsetmacro{\warpSizeN}{\sizePerThreadN*\threadsPerWarpN}

  \foreach \tid in {0,...,63}{
    \pgfmathsetmacro{\tidM}{int(\tid/\threadsPerWarpN)}
    \pgfmathsetmacro{\tidN}{mod(\tid,\threadsPerWarpN)}
    \coordinate (Thread TL) at ($(Warp TL)+(\tidN*\sizePerThreadN*\elem, -\tidM*\sizePerThreadM*\elem)$);
  ...
  \draw (Warp TL) rectangle ++(\warpSizeN*\elem, -\warpSizeM*\elem);
}
...
\newcommand{\drawBlockedCTA}[7]{
  ...
  \pgfmathsetmacro{\maxWarpId}{\warpsPerCTAM*\warpsPerCTAN-1}

  \coordinate (Warp TL) at (CTA TL);
  \drawBlockedWarp{\sizePerThreadM}{\sizePerThreadN}{\threadsPerWarpM}{\threadsPerWarpN}{\order}
  \foreach \warpId in {0,...,\maxWarpId}{
    \ifthenelse{\order=1}
    {
      \pgfmathsetmacro{\warpCoordM}{int(\warpId/\warpsPerCTAN)}
      \pgfmathsetmacro{\warpCoordN}{mod(\warpId,\warpsPerCTAN)}
      \pgfmathsetmacro{\rot}{0}
    }{
      \pgfmathsetmacro{\warpCoordM}{mod(\warpId,\warpsPerCTAM)}
      \pgfmathsetmacro{\warpCoordN}{int(\warpId/\warpsPerCTAM)}
      \pgfmathsetmacro{\rot}{90}
    }
    
    \coordinate (Warp TL) at ($(CTA TL)+(\warpCoordN*\warpSizeN*\elem, -\warpCoordM*\warpSizeM*\elem)$);
    \draw [ultra thin] (Warp TL) rectangle ++(\warpSizeN*\elem, -\warpSizeM*\elem)
    node [pos=.5, scale=.6*\scale, inner sep=0, fill=white, rotate=\rot] {warp\warpId};
  }
...
\newcommand{\drawBlockMFMALayoutLarge}[2]{
  ...
  \foreach \iVec in {0,1,2,3} {
    \coordinate (warp TL) at ($(block TL)+(\trans*\iVec*2*4*\elem, -\nonTrans*\iVec*2*4*\elem)$);
...
\newcommand{\drawTensorMFMALayout}[6]{
  ...
  \pgfmathsetmacro{\maxWarpId}{\warpsPerCTAH*\warpsPerCTAW-1}
  ...
  \foreach \ctaId in {0,...,\maxCTAId}{
    ...
    %% Draw a detailed view of warp0 in each CTA
    \coordinate (block TL) at (CTA TL);
    \drawBlockMFMALayoutLarge{\mfmaTrans}{0}
    
    \foreach \warpId in {0,...,\maxWarpId}{
      \pgfmathsetmacro{\warpCoordH}{int(\warpId/\warpsPerCTAW)}
      \pgfmathsetmacro{\warpCoordW}{mod(\warpId,\warpsPerCTAW)}
      \coordinate (block TL) at ($(CTA TL)+(\warpCoordW*\mfmaNonKDim*\elem, -\warpCoordH*\mfmaNonKDim*\elem)$);
      %% Inside the loop, only draw a rectangle
      \draw [ultra thin] (block TL) rectangle ++(\mfmaNonKDim*\elem, -\mfmaNonKDim*\elem)
      node [scale=.7*\scale, pos=.5, fill=white, inner sep=0] {warp\warpId};
    }
...
\newcommand{\drawWarpOperand}[4]{
  %%
  %% Draw the part of the tensor that is one operand of the warp
  %%
...
\newcommand{\drawDotOperands}[7]{
  ...
  \pgfmathsetmacro{\maxWarpId}{\warpsPerCTAM-1}
  \foreach \ctaId in {0,...,\maxCTAIdM}{
    \coordinate (CTA TL) at ($(A TL)+(0, -\ctaId*\warpsPerCTAM*32*\elem)$);
    \foreach \warpId in {0,...,\maxWarpId}{
      \coordinate (warp TL) at ($(CTA TL)+(0, -\warpId*32*\elem)$);
      \draw [ultra thin] (warp TL) rectangle ++(\K*\elem, -32*\elem);
    }
    %% Only draw the detailed view of the first warp in CTA
    \coordinate (Op TL) at (CTA TL);
    \drawWarpOperand{\K}{\mfmaNonKDim}{\kpack}{0}
  ...
  \pgfmathsetmacro{\maxWarpId}{\warpsPerCTAN-1}
  \foreach \ctaId in {0,...,\maxCTAIdN}{
    \coordinate (CTA TL) at ($(B TL)+(\ctaId*\warpsPerCTAN*32*\elem, 0)$);
    \foreach \warpId in {0,...,\maxWarpId}{
      \coordinate (warp TL) at ($(CTA TL)+(\warpId*32*\elem ,0)$);
      \draw [ultra thin] (warp TL) rectangle ++(32*\elem, -\K*\elem);
    }
    %% Only draw the detailed view of the first warp in CTA
    \coordinate (Op TL) at (CTA TL);
    \drawWarpOperand{\K}{\mfmaNonKDim}{\kpack}{1}
