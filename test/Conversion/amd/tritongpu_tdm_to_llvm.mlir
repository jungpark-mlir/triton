...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @tdm_prefetch_regular(%arg0: !tt.ptr<f16> {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    ...
    amdg.tdm_prefetch %0[%c_offset, %c_offset], %c_pred, speculative = false, in_bound = false : !tt.tensordesc<64x64xf16, #shared>

    // CHECK: rocdl.global.prefetch %{{.*}}, 8 : !llvm.ptr<1>
    amdg.tdm_prefetch %0[%c_offset, %c_offset], %c_pred, speculative = true, in_bound = false : !tt.tensordesc<64x64xf16, #shared>

    // CHECK: rocdl.global.prefetch %{{.*}}, 9 : !llvm.ptr<1>
    amdg.tdm_prefetch %0[%c_offset, %c_offset], %c_pred, speculative = false, in_bound = true : !tt.tensordesc<64x64xf16, #shared>
    tt.return
  }
}
