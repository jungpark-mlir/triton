...
def _store_quadrants(c_ptr, pid_m, pid_n, stride_cm, stride_cn, M, N, acc_tl, acc_bl, acc_tr, acc_br,
    ...
    gl.amd.cdna5.buffer_store(gl.convert_layout(acc_tl.to(c_ptr.type.element_ty), STORE_LAYOUT), base, quad_offsets,
                                mask=mask_tl)
    gl.amd.cdna5.buffer_store(gl.convert_layout(acc_bl.to(c_ptr.type.element_ty), STORE_LAYOUT),
                                base + (BLOCK_M // 2) * stride_cm, quad_offsets, mask=mask_bl)
    gl.amd.cdna5.buffer_store(gl.convert_layout(acc_tr.to(c_ptr.type.element_ty), STORE_LAYOUT),
                                base + (BLOCK_N // 2) * stride_cn, quad_offsets, mask=mask_tr)
    gl.amd.cdna5.buffer_store(gl.convert_layout(acc_br.to(c_ptr.type.element_ty), STORE_LAYOUT),
...
@gluon.jit
def f16_slice_mn_warp_pipeline_kernel_cdna5(a_ptr, b_ptr, c_ptr, M, N, K, stride_am, stride_ak, stride_bk,
    ...
    if RESOLVE_PARTITION_CONFLICTS:
        layouts: gl.constexpr = gl.amd.cdna5.make_partitioned_dot_layouts(
    ...
    for _ in range(0, iter_max - (NUM_BUFFERS - 1)):
        ...
        with gl.amd.warp_pipeline_stage("mfma", priority=0):
            acc_tl = gl.amd.cdna5.wmma(a_top, b_left, acc_tl)
        ...
        with gl.amd.warp_pipeline_stage("mfma", priority=0):
            acc_bl = gl.amd.cdna5.wmma(a_bot, b_left, acc_bl)
        tdm.async_wait(steady_wait)
        with gl.amd.warp_pipeline_stage("mem", priority=1):
        ...
        with gl.amd.warp_pipeline_stage("mfma", priority=0):
            acc_tr = gl.amd.cdna5.wmma(a_top, b_right, acc_tr)
        tdm.async_wait(steady_wait)
        with gl.amd.warp_pipeline_stage("mem", priority=1):
        ...
        with gl.amd.warp_pipeline_stage("mfma", priority=0):
            acc_br = gl.amd.cdna5.wmma(a_bot, b_right, acc_br)
        tdm.async_wait(steady_wait)
        with gl.amd.warp_pipeline_stage("mem", priority=1):
...
@gluon.jit
def mxfp_slice_mn_warp_pipeline_kernel_cdna5(a_ptr, b_ptr, c_ptr, a_scale_ptr, b_scale_ptr, M, N, K, stride_am,
    ...
    scale_a_layout: gl.constexpr = gl.amd.cdna5.get_wmma_scale_layout(dot_a, [HALF_M, BK_SCALE])
    scale_b_layout: gl.constexpr = gl.amd.cdna5.get_wmma_scale_layout(dot_b, [HALF_N, BK_SCALE])
    ...
    for _ in range(0, iter_max - (NUM_BUFFERS - 1)):
        ...
        next_slot = (consume_k + 1) % nbuf
        write_slot = load_k % nbuf
        acc_tl = gl.amd.cdna5.wmma_scaled(a_top, as_top, DTYPE_A, b_left, bs_left, DTYPE_B, acc_tl)
        ...
        acc_bl = gl.amd.cdna5.wmma_scaled(a_bot, as_bot, DTYPE_A, b_left, bs_left, DTYPE_B, acc_bl)
        _wait_mxfp_scale_pipeline(steady_wait, ASYNC_COPY_SCALE)
        b_right = b_right_buf.index(read_slot).permute([1, 0]).load(layout=dot_b)
        ...
        acc_tr = gl.amd.cdna5.wmma_scaled(a_top, as_top, DTYPE_A, b_right, bs_right, DTYPE_B, acc_tr)
        _wait_mxfp_scale_pipeline(steady_wait, ASYNC_COPY_SCALE)
        b_left = b_left_buf.index(next_slot).permute([1, 0]).load(layout=dot_b)
        ...
        acc_br = gl.amd.cdna5.wmma_scaled(a_bot, as_bot, DTYPE_A, b_right, bs_right, DTYPE_B, acc_br)
        _wait_mxfp_scale_pipeline(steady_wait, ASYNC_COPY_SCALE)
        a_top = a_top_buf.index(next_slot).load(layout=dot_a)
        if WITH_A_SCALE:
...
def _make_f16_case(args):
    ...
    def launch():
        return f16_slice_mn_warp_pipeline_kernel_cdna5[grid](
...
def _make_mxfp_case(args):
    ...
    def launch():
        return mxfp_slice_mn_warp_pipeline_kernel_cdna5[grid](
...
def _build_arg_parser():
    parser = argparse.ArgumentParser(description="cdna5 warp-pipelined GEMM examples for 16/8/4-bit inputs")
    ...
    parser.add_argument("--resolve-partition-conflicts", action="store_true",
                        help="Use partition-aware cdna5 WMMA/shared layouts")
