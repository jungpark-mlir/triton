...
def gemm_tdm_pipelined_warp_pipelined_kernel(a_ptr, b_ptr, c_ptr,  #
    ...
    for _ in range(0, ttgl.cdiv(K, BLOCK_K) - (NUM_BUFFERS - 1)):
        ...
        with ttgl.amd.warp_pipeline_stage("stage1", priority=0):
            issue_loads(write_phase, a_desc, b_desc, 0, 0, a_buffer, b_buffer, BLOCK_K, NUM_BUFFERS,
                        TRANSPOSE_B)
    ...
    for i in ttgl.static_range(NUM_BUFFERS - 1):
        ...
        phase, accumulator = issue_wmma(phase, a_buffer, OPERAND_LAYOUT_A, b_buffer, OPERAND_LAYOUT_B,
...
# ---------------------------------------------------------------------------
# KernelB variant: loads+TDM in stage0, dot-only in stage1 (num_buffers=4).
#
...
def gemm_tdm_specialized_pipelined_warp_pipelined_kernel(a_ptr, b_ptr, c_ptr,  #
    ...
    for _ in range(0, ttgl.cdiv(K, BLOCK_K) - (NUM_BUFFERS - 1)):
        with ttgl.amd.warp_pipeline_stage("stage0", priority=0):
            ...
            phase, a, b = lds_load(phase, a_buffer, OPERAND_LAYOUT_A, b_buffer, OPERAND_LAYOUT_B, NUM_BUFFERS,
                                   TRANSPOSE_B)
            issue_loads_specialized(write_phase, a_desc, b_desc, 0, 0, a_buffer, b_buffer, BLOCK_K,
...
@pytest.mark.parametrize("BLOCK_M,BLOCK_N,BLOCK_K", [(256, 256, 64)])
@pytest.mark.parametrize("NUM_BUFFERS", [4])
@pytest.mark.parametrize("TRANSPOSE_B", [True])
@pytest.mark.parametrize("M,N,K", [(2048, 2048, 2048)])
@pytest.mark.parametrize("DUMP", [False])
def test_runtime_gemm_tdm_pipelined_B(BLOCK_M, BLOCK_N, BLOCK_K, NUM_BUFFERS, TRANSPOSE_B, M, N, K, DUMP):
    if triton.cdiv(K, BLOCK_K) < NUM_BUFFERS:
        pytest.skip("Skip tests where K/BLOCK_K < NUM_BUFFERS")
    ...
    warp_bases = tuple(WARP_BASES)
    grid = (triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1)
    kernel = gemm_tdm_pipelined_warp_pipelined_kernelB[grid](
        a_device, b_device, c_device,  #
        M, N, K,  #
        stride_am, stride_ak,  #
        stride_bk, stride_bn,  #
        ...
        BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N, BLOCK_K=BLOCK_K,  #
        NUM_BUFFERS=NUM_BUFFERS, TRANSPOSE_B=TRANSPOSE_B, WARP_BASES=warp_bases,  #
        PRIO_SWAP=(os.environ.get('WMMA_B2B') == '1'),  #
        num_warps=num_warps, waves_per_eu=num_warps // 4)
    static_profile(kernel)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kernelB", action="store_true",
                        help="Use kernelB schedule (loads+TDM in stage0, dot in stage1)")
    ...
    if args.kernelB:
        test_runtime_gemm_tdm_pipelined_B(BLOCK_M, BLOCK_N, BLOCK_K, NUM_BUFFERS, TRANSPOSE_B, M, N, K, DUMP)
    elif args.four_warp_tdm:
